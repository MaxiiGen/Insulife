...
import { predict, PredictionResult, PatientInput } from "@/lib/predict";
      ...
      <form onSubmit={onSubmit} className="rounded-[var(--radius)] bg-card p-8 shadow-[var(--shadow-card)]">
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="space-y-2">
            <Label htmlFor="Sex" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              Sex
            </Label>
            <select
              id="Sex"
              value={values.Sex}
              onChange={(e) => setValues({ ...values, Sex: Number(e.target.value) })}
              className="flex h-12 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm ring-offset-background"
              required
            >
...
const ResultPanel = ({ result }: { result: PredictionResult | null }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-[var(--radius)] bg-[image:var(--gradient-hero)] p-10 text-primary-foreground shadow-[var(--shadow-soft)]">
      ...
      <p className="mt-6 text-xs opacity-60">Source: XGBoost model.pkl</p>
    </div>
  );
};
